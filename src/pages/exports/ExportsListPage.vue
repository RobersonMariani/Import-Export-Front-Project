...
const allSelected = computed(
  () =>
    exportStore.exports.length > 0 &&
    exportStore.exports.every((e) => selectedIds.value.has(e.id)),
);
...
function toggleSelectAll(): void {
  if (allSelected.value) {
    ...
    selectedIds.value = new Set(exportStore.exports.map((e) => e.id));
  }
}
...
async function handleBulkDelete(): Promise<void> {
  ...
  if (count === 0) return;
  if (!confirm(`Excluir ${count} exportação(ões) selecionada(s)?`)) return;

  bulkDeleting.value = true;
  try {
    await exportStore.bulkDeleteExports([...selectedIds.value]);
    selectedIds.value = new Set();
  } finally {
  ...
  <div>
    ...
    <AppCard :padding="false">
      ...
      >
        ...
        >
          <div class="flex items-center gap-3 rounded-lg bg-red-50 px-4 py-2">
